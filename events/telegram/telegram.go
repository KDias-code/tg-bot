...
func New(client *telegram.Client, storage files.Storage) *Proc {
	return &Proc{
		tg:      client,
		storage: storage,
	}
...
func (p *Proc) Process(event events.Event) error {
	...
	default:
		return err2.Wrap("can't process message", ErrUnknownEventType)
	}
}
...
func (p *Proc) processMessage(event events.Event) error {
	meta, err := meta(event)
	if err != nil {
		return err2.Wrap("cant process message", err)
	}
...
func meta(event events.Event) (Meta, error) {
	...
	if !ok {
		return Meta{}, err2.Wrap("can't get meta", ErrUnknownMetaType)
	}
